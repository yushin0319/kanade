...
import { buildSystemInstruction, parseBriefing } from '../lib/briefing-loader'
...
export function useLiveApi(options: UseLiveApiOptions = {}): UseLiveApiReturn {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      const briefingText =
        briefingJson.status === 'fulfilled' ? briefingJson.value : null
      const promptText =
        systemPrompt.status === 'fulfilled' ? systemPrompt.value : undefined
      ...
      if (briefingText) {
        ...
        console.log(
          'System prompt loaded (no briefing):',
          systemInstruction.length,
          'chars',
        )
      }
