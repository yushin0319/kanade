...
import { writeSummary, writeConversation } from "../lib/summary-writer";
import { notifyCompletion, injectToCC } from "../lib/cc-notifier";
    ...
    return (
      <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
        <span
          style={{
            fontSize: "0.8rem",
            color: sendState === "error" ? "#f44" : sendState === "done" ? "#4caf50" : "#aaa",
          }}
        >
          {statusText[sendState]}
        </span>
        {sendState === "error" && (
          <button onClick={handleCancel} style={smallButtonStyle}>
            閉じる
          </button>
        )}
      </div>
